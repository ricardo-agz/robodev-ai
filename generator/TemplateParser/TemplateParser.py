...
class TemplateParser:
                    ...
                    for x in range(len(eval_arr)):
                        ...
                        for b_index in range(len(new_block)):
                            ...
                            if "<$=" in block_line and "$>" in block_line:
                                ...
                                temp_line = block_line[:block_line.find("<$=") + 3] + evaluated + block_line[
                                                                                                  block_line.find(
                                                                                                      "$>"):]
    ...
    def insert_var_in_line(self, line, many_model=None, alias=None):
        ...
        if "$$singleAliasCamel$$" in line:
            line = line.replace("$$singleAliasCamel$$",
                                camel_case(singularize(alias)) if alias else camel_case(many_name))
            out = True
        if "$$MailerName$$" in line:
            line = line.replace("$$MailerName$$", pascal_case(self.mailer.name))
            out = True
        if "$$header$$" in line:
